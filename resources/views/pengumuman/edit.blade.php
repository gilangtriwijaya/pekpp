...
@section('title', 'PEKPPP — Edit Pengumuman')

@section('content')
<div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 max-w-3xl mx-auto">
    <div class="mb-6">
        <h1 class="text-xl font-bold text-slate-800">Edit Pengumuman</h1>
        <p class="text-sm text-slate-500 mt-0.5">Perbarui informasi pengumuman yang sudah dipublikasikan</p>
    </div>
        ...
        <div class="space-y-5">
            ...
            <div>
                ...
                <textarea name="isi" id="isi" rows="6" required
                          ...
                          placeholder="Masukkan teks pengumuman secara detail...">{{ old('isi', $pengumuman->isi) }}</textarea>
                @error('isi')
                    <p class="text-rose-600 text-xs font-semibold mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="datetime-local" name="published_at" id="published_at" 
                           value="{{ old('published_at', $pengumuman->published_at ? $pengumuman->published_at->format('Y-m-d\TH:i') : '') }}"
                ...
                <div>
                    ...
                    <input type="datetime-local" name="expired_at" id="expired_at" 
                           value="{{ old('expired_at', $pengumuman->expired_at ? $pengumuman->expired_at->format('Y-m-d\TH:i') : '') }}"
            ...
            <div class="flex items-center gap-2.5 bg-slate-50 border border-slate-200 rounded-lg p-4">
                <input type="checkbox" name="aktif" id="aktif" value="1" {{ old('aktif', $pengumuman->aktif) ? 'checked' : '' }}
